...
fun_calc_pdetect_multi_both = function(x,
  ...
  if(model_type == "R0"){
    ...
    # res returns the total Nb of infection on the whole period, I only need 1984 to 2017
    Ninf = rowSums( res$Ninf_t_province[,which(dim_year==1984):which(dim_year==2017)] )
